...
class CtgDefaults(BaseModel):
    ...
    default_unit_of_measure: Annotated[Optional[UnitOfMeasure], Field(description='Deafult unit of measure')] = None
    ...
    def validate_tracking_type_units(self) -> 'CtgDefaults':
        ...
        if self.default_tracking_type == ProductTrackingType.WEIGHT_BASED:
            ...
            if self.default_unit_of_measure not in allowed:
                raise ValueError(
                    f'Weight-based tracking requires unit_of_measure in {allowed}, got {self.default_unit_of_measure}'
                )
        # Piece
